...
1. **하이브리드 AI 파이프라인**:
   ...
   - **단어 단위 정밀 타임스탬프 (`word_timestamps`)**: 30초 통짜 블록 대신 자연스러운 문장 단위(마침표/물음표) 및 적정 길이(최대 5.5초)로 스마트 분할하여 문맥 파절/단어 중간 끊김 방지
   - **번역 및 텍스트 교정**: `Google Gemini Flash (3.5 / 3.6 / 3.7)` 기반 문맥 의역 + 한국어 맞춤법/띄어쓰기 정제
   - **다중 화자 대화 포맷팅**: 한 화면 내 다중 대화 시 슬래시(/) 대신 넷플릭스 표준 하이픈(`- `) 자동 줄바꿈 포맷팅 적용 (불필요한 `[화자]` 태그 배제)
2. **다양한 작업 모드**:
   - **자막 없는 동영상**: 오디오 추출 ➔ 정밀 음성인식(STT) ➔ Gemini 번역/교정 ➔ `.ko.srt` 생성
